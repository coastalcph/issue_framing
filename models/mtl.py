...
from models.layers import SoftmaxLayer, EmbeddingLayer, RnnLayer
...
"""
A multi-task model for sequence classification
"""
class MTL_model(object):

    def __init__(self, num_layers, input_dim, hidden_dim, tasks, main_task, vocabularies, update_embs, exp_path, prediction_layer, additional_params):
        ...
        # iterate over a sorted dict to make the results reproducible
        ...
        for tid, task in sorted(iter(self.tasks.items())):
            if task.trainable:
                print('Adding output layer: {}'.format(tid))
    ...
    def train_batched(self, tasks, batch_size, validation_data, early_stopping, patience, num_epochs,
                           min_num_epochs, num_updates, prob_main_task):
    ...
    def save(self, path):
        ...
        # remove the old modelfile
        if os.path.isfile(abs_basename):
            os.remove(abs_basename)
    ...
    def sample_task_batch(self, batch_dict, prob_main_task):
        ...
        other_task_ids = [elm for elm in sorted(iter(self.tasks.keys())) if elm != main_task_id and  self.tasks[elm].trainable]
