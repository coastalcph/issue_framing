...
def load_data(config, main_task, aux_tasks, concat_datasets):
    """
...
def setup_vocabularies(args, task_names, task2data, share_vocabs, wembed_files, setting):
    ...
    if setting == 'development':
        for voc, tids in sorted(iter(embs2task.items())):
            ...
            vocabularies[voc] = mtl.fit_sequences(seqs, wembed_file=wembed_files[voc])
...
def setup_task(task_name, data_name, data, vocab_builder, update_embeds, vocab_name, setting):
    ...
    for ds in ['train', 'dev', 'test']:
        ...
        input[ds]['seq'] = mtl.transform_sequences(data[ds]['seq'], vocab_builder)
        input[ds]['label'] = mtl.prepare_labels(data[ds]['label'], data['train']['labelset'])[1]
    ...
    if setting == 'development':
        ...
        trainable=len(input['train']['seq'])>0
        task = Task(task_name=task_name, data=input_merged, vocab_size=vocab_builder.vocab_size, update_embs=update_embeds,
                        vocab_name=vocab_name, trainable=trainable)
        return task
...
def main(args):
    ...
    dynet_params = setup_dynet(random_seed=args.seed, weight_decay=args.weight_decay, mem=args.mem, cuda=args.cuda)
    ...
    task2data = load_data(config, args.main_task, args.aux_tasks, args.concat)
    ...
    if args.setting=='development':
        ...
        # set up the model
        net = mtl.MTL_model(num_layers=args.num_layers, input_dim=args.input_dim, hidden_dim=args.hidden_dim,
                            vocabularies=vocabularies, update_embs=args.update_wembeds, tasks=tasks, main_task=args.main_task,
        ...
        final_epoch, final_f_dev, sum_train_losses, no_improvement, best_f_dev = \
            net.train_batched(tasks=tasks, batch_size=args.batch_size, validation_data=validation_data, num_epochs=args.num_epochs, early_stopping=args.early_stopping,
                              patience=args.patience, min_num_epochs=args.min_epochs, num_updates=args.num_updates, prob_main_task=args.prob_main_task)
        """
        # try lpr
        counter = 0
        for i, seq in enumerate(tasks[args.main_task].dev_seqs):
            if counter > 10:
                break
            counter += 1
            label = tasks[args.main_task].dev_labels[i]
            target_class = np.argmax(label)
            print('Real label: {} {}'.format(target_class, tasks[args.main_task].labelset[target_class]))
            net.lrp(seq, task=args.main_task, target_class=target_class)
        """
        ...
        for tid, task in tasks.items():
            ...
            if len(task.train_seqs) == 0:
                ...
                res_train = mtl.evaluate_model_predictions(preds_train, task.train_labels, task.labelset)

            preds_dev = net.predict(tid, task.dev_seqs)
            write_model_predictions(preds_dev, pred_file)
            res_dev = mtl.evaluate_model_predictions(preds_dev, task.dev_labels, task.labelset)
        ...
        if args.retrain:
            ...
            # set up the model
            net = mtl.MTL_model(num_layers=loaded_model_params['num_layers'], input_dim=loaded_model_params['input_dim'], hidden_dim=loaded_model_params['hidden_dim'],
                                ...
                                additional_params={'dynet_params': loaded_model_params['dynet_params']})
        ...
        for tid, task in tasks.items():
            ...
            res_test = mtl.evaluate_model_predictions(preds_test, task.test_labels, task.labelset)
...
def log_results(task_name, train, test, labelset, setting, result_file, params, final_epoch='testing', train_loss='testing', best_dev_f='testing'):
    with open(result_file, 'a') as f:
        ...
        f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(task_name, 'macro_avg', train['p_macro_avg'],
                                                                          ...
                                                                          test['p_macro_avg'], test['r_macro_avg'], test['f_macro_avg'],
...
if __name__=="__main__":
    ...
    parser.add_argument('--cuda', action='store_true',
                        help="Run on GPU")
    #parser.add_argument('--combination', type=int, help="The combination of datasets for auxiliary and main task")
